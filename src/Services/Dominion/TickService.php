...
class TickService
{
    ...
    public function performTick(Round $round, Dominion|null $dominion = null)
    {
        ...
        DB::transaction(function () use ($where) {
            ...
            if (DB::getDriverName() === 'sqlite') {
                $this->performTickDominionsSqlite($where);
            } else {
                DB::table('dominions')
            ...
            // Update queues
            if (DB::getDriverName() === 'sqlite') {
                $this->performTickQueuesSqlite($where);
            } else {
    ...
    /**
     * SQLite-compatible queue tick update.
     *
     * SQLite checks PRIMARY KEY constraints per-row during UPDATE. When consecutive
     * hours exist for the same (dominion_id, source, resource), a direct "hours - 1"
     * causes transient conflicts (e.g. hours=3→2 collides with an existing hours=2 row).
     * Fix: shift all hours up by a large offset first, then subtract offset+1.
     */
    private function performTickQueuesSqlite(array $where): void
    {
        $dominionIds = DB::table('dominions')->where($where)->pluck('id')->all();
        $offset = 1000000;
        DB::table('dominion_queue')
            ->whereIn('dominion_id', $dominionIds)
            ->update(['hours' => DB::raw('hours + ' . $offset)]);
        DB::table('dominion_queue')
            ->whereIn('dominion_id', $dominionIds)
            ->update([
                'hours' => DB::raw('hours - ' . ($offset + 1)),
                'updated_at' => $this->now,
            ]);
    }

    /**
     * SQLite-compatible dominion tick update.
     *
     * SQLite does not support UPDATE...JOIN syntax. We resolve the matching
     * dominion IDs first, then apply a single bulk UPDATE...FROM statement
     * (available since SQLite 3.33, bundled with PHP 8.x) to read delta values
     * directly from dominion_tick — equivalent to the MySQL JOIN UPDATE.
     */
    private function performTickDominionsSqlite(array $where): void
    {
        $dominionIds = DB::table('dominion_tick')
            ->join('dominions', 'dominions.id', '=', 'dominion_tick.dominion_id')
            ->where($where)
            ->where(function ($query) {
                $query->where('dominions.abandoned_at', null)->orWhere('dominions.abandoned_at', '>', $this->now);
            })
            ->pluck('dominion_tick.dominion_id')
            ->all();

        if (empty($dominionIds)) {
            return;
        }

        $placeholders = implode(',', array_fill(0, count($dominionIds), '?'));

        DB::statement("
            UPDATE dominions
            SET
                prestige                    = dominions.prestige + dominion_tick.prestige,
                peasants                    = dominions.peasants + dominion_tick.peasants,
                peasants_last_hour          = dominion_tick.peasants,
                morale                      = dominions.morale + dominion_tick.morale,
                spy_strength                = dominions.spy_strength + dominion_tick.spy_strength,
                wizard_strength             = dominions.wizard_strength + dominion_tick.wizard_strength,
                resilience                  = dominions.resilience + dominion_tick.resilience,
                fireball_meter              = dominions.fireball_meter + dominion_tick.fireball_meter,
                lightning_bolt_meter        = dominions.lightning_bolt_meter + dominion_tick.lightning_bolt_meter,
                resource_platinum           = dominions.resource_platinum + dominion_tick.resource_platinum,
                resource_food               = dominions.resource_food + dominion_tick.resource_food,
                resource_lumber             = dominions.resource_lumber + dominion_tick.resource_lumber,
                resource_mana               = dominions.resource_mana + dominion_tick.resource_mana,
                resource_ore                = dominions.resource_ore + dominion_tick.resource_ore,
                resource_gems               = dominions.resource_gems + dominion_tick.resource_gems,
                resource_tech               = dominions.resource_tech + dominion_tick.resource_tech,
                resource_boats              = dominions.resource_boats + dominion_tick.resource_boats,
                improvement_science         = dominions.improvement_science + dominion_tick.improvement_science,
                improvement_keep            = dominions.improvement_keep + dominion_tick.improvement_keep,
                improvement_forges          = dominions.improvement_forges + dominion_tick.improvement_forges,
                improvement_walls           = dominions.improvement_walls + dominion_tick.improvement_walls,
                military_draftees           = dominions.military_draftees + dominion_tick.military_draftees,
                military_unit1              = dominions.military_unit1 + dominion_tick.military_unit1,
                military_unit2              = dominions.military_unit2 + dominion_tick.military_unit2,
                military_unit3              = dominions.military_unit3 + dominion_tick.military_unit3,
                military_unit4              = dominions.military_unit4 + dominion_tick.military_unit4,
                military_spies              = dominions.military_spies + dominion_tick.military_spies,
                military_assassins          = dominions.military_assassins + dominion_tick.military_assassins,
                military_wizards            = dominions.military_wizards + dominion_tick.military_wizards,
                military_archmages          = dominions.military_archmages + dominion_tick.military_archmages,
                land_plain                  = dominions.land_plain + dominion_tick.land_plain,
                land_mountain               = dominions.land_mountain + dominion_tick.land_mountain,
                land_swamp                  = dominions.land_swamp + dominion_tick.land_swamp,
                land_cavern                 = dominions.land_cavern + dominion_tick.land_cavern,
                land_forest                 = dominions.land_forest + dominion_tick.land_forest,
                land_hill                   = dominions.land_hill + dominion_tick.land_hill,
                land_water                  = dominions.land_water + dominion_tick.land_water,
                discounted_land             = dominions.discounted_land + dominion_tick.discounted_land,
                building_home               = dominions.building_home + dominion_tick.building_home,
                building_alchemy            = dominions.building_alchemy + dominion_tick.building_alchemy,
                building_farm               = dominions.building_farm + dominion_tick.building_farm,
                building_smithy             = dominions.building_smithy + dominion_tick.building_smithy,
                building_masonry            = dominions.building_masonry + dominion_tick.building_masonry,
                building_ore_mine           = dominions.building_ore_mine + dominion_tick.building_ore_mine,
                building_gryphon_nest       = dominions.building_gryphon_nest + dominion_tick.building_gryphon_nest,
                building_tower              = dominions.building_tower + dominion_tick.building_tower,
                building_wizard_guild       = dominions.building_wizard_guild + dominion_tick.building_wizard_guild,
                building_temple             = dominions.building_temple + dominion_tick.building_temple,
                building_diamond_mine       = dominions.building_diamond_mine + dominion_tick.building_diamond_mine,
                building_school             = dominions.building_school + dominion_tick.building_school,
                building_lumberyard         = dominions.building_lumberyard + dominion_tick.building_lumberyard,
                building_forest_haven       = dominions.building_forest_haven + dominion_tick.building_forest_haven,
                building_factory            = dominions.building_factory + dominion_tick.building_factory,
                building_guard_tower        = dominions.building_guard_tower + dominion_tick.building_guard_tower,
                building_shrine             = dominions.building_shrine + dominion_tick.building_shrine,
                building_barracks           = dominions.building_barracks + dominion_tick.building_barracks,
                building_dock               = dominions.building_dock + dominion_tick.building_dock,
                stat_total_platinum_production  = dominions.stat_total_platinum_production + dominion_tick.resource_platinum,
                stat_total_food_production      = dominions.stat_total_food_production + dominion_tick.resource_food_production,
                stat_total_lumber_production    = dominions.stat_total_lumber_production + dominion_tick.resource_lumber_production,
                stat_total_mana_production      = dominions.stat_total_mana_production + dominion_tick.resource_mana_production,
                stat_total_ore_production       = dominions.stat_total_ore_production + dominion_tick.resource_ore,
                stat_total_gem_production       = dominions.stat_total_gem_production + dominion_tick.resource_gems,
                stat_total_tech_production      = dominions.stat_total_tech_production + dominion_tick.resource_tech,
                stat_total_boat_production      = dominions.stat_total_boat_production + dominion_tick.resource_boat_production,
                stat_total_food_decay           = dominions.stat_total_food_decay + dominion_tick.resource_food_decay,
                stat_total_lumber_decay         = dominions.stat_total_lumber_decay + dominion_tick.resource_lumber_decay,
                stat_total_mana_decay           = dominions.stat_total_mana_decay + dominion_tick.resource_mana_decay,
                highest_land_achieved       = dominions.highest_land_achieved + dominion_tick.highest_land_achieved,
                calculated_networth         = dominion_tick.calculated_networth,
                last_tick_at                = ?
            FROM dominion_tick
            WHERE dominions.id = dominion_tick.dominion_id
            AND dominions.id IN ($placeholders)
        ", array_merge([(string) $this->now], $dominionIds));
    }
